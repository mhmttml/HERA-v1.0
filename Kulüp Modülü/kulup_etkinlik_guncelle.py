from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QLabel, QLineEdit, QPushButton, 
                             QHBoxLayout, QGraphicsDropShadowEffect, QWidget, QComboBox,
                             QTextEdit, QDateEdit, QMessageBox, QGridLayout, QCalendarWidget)
from PyQt6.QtGui import QFont, QColor, QIntValidator
from PyQt6.QtCore import Qt, QLocale, QDate, QTime
...
class KulupEtkinlikGuncelleDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # Başlık
        title = QLabel("Etkinlik Güncelle")
    ...
    def load_etkinlik_data(self):
        """Mevcut etkinlik verilerini form alanlarına yükle"""
        if self.etkinlik_data:
            # Etkinlik ID (değiştirilemez)
            self.etkinlik_id_edit.setText(self.etkinlik_data.get('etkinlik_id', ''))
            
            # Diğer alanları doldur
            self.etkinlik_adi_edit.setText(self.etkinlik_data.get('etkinlik_adi', ''))
            
            # Kulüp ComboBox'ına mevcut değeri ekle ve seç
            current_kulup = self.etkinlik_data.get('kulup', '')
            if current_kulup and current_kulup not in [self.kulup_combo.itemText(i) for i in range(self.kulup_combo.count())]:
                self.kulup_combo.addItem(current_kulup)
            self.kulup_combo.setCurrentText(current_kulup)
            
            # Etkinlik durumu
            current_durum = self.etkinlik_data.get('etkinlik_durumu', '')
            if current_durum:
                self.durum_combo.setCurrentText(current_durum)
            
            # Etkinlik lideri
            current_lider = self.etkinlik_data.get('etkinlik_lideri', '')
            if current_lider and current_lider not in [self.lider_combo.itemText(i) for i in range(self.lider_combo.count())]:
                self.lider_combo.addItem(current_lider)
            self.lider_combo.setCurrentText(current_lider)
            
            # Etkinlik açıklaması
            self.aciklama_edit.setText(self.etkinlik_data.get('etkinlik_aciklamasi', ''))
            
            # Katılımcı sayıları
            self.min_katilimci_edit.setText(str(self.etkinlik_data.get('min_katilimci', '')))
            self.max_katilimci_edit.setText(str(self.etkinlik_data.get('max_katilimci', '')))
            
            # Tarihler
            baslangic_tarih = self.etkinlik_data.get('baslangic_tarihi', '')
            if baslangic_tarih:
                self.baslangic_edit.setText(baslangic_tarih)
            
            bitis_tarih = self.etkinlik_data.get('bitis_tarihi', '')
            if bitis_tarih:
                self.bitis_edit.setText(bitis_tarih)
    ...
    def get_updated_etkinlik_data(self):
        """Güncellenmiş etkinlik verilerini döndür"""
        return {
            'etkinlik_id': self.etkinlik_id_edit.text(),
            'etkinlik_adi': self.etkinlik_adi_edit.text(),
            'kulup': self.kulup_combo.currentText(),
            'etkinlik_durumu': self.durum_combo.currentText(),
            'etkinlik_lideri': self.lider_combo.currentText(),
            'etkinlik_aciklamasi': self.aciklama_edit.text(),
